...
def remove_russian_text(text):
    """Удалить русский текст, оставить только английский для озвучки"""
    # Удаляем блоки с переводами (между ---)
    text = re.sub(r'---.*?---', '', text, flags=re.DOTALL)

    # Удаляем русский текст в скобках: (Russian text)
    text = re.sub(r'\([^()]*[а-яА-ЯёЁ][^()]*\)', '', text)

    # Удаляем русский текст после тире: - Russian text
    text = re.sub(r'-\s*[а-яА-ЯёЁ][^\n.!?]*', '', text)

    # Удаляем строки содержащие кириллицу
    lines = text.split('\n')
    english_lines = []
    for line in lines:
        # Если в строке есть кириллица - пропускаем
...
def text_to_speech(text, output_path=None):
    ...
    """
    try:
        # УБИРАЕМ ЭМОДЗИ и РУССКИЙ ТЕКСТ перед генерацией голоса
        clean_text = remove_emojis(text)
        clean_text = remove_russian_text(clean_text)
        clean_text = clean_text.strip()

        if not clean_text:
            print("⚠️ Текст пустой после фильтрации, пропускаю генерацию голоса")
            return None

        print(f"🎤 Генерирую голос через Google TTS (только English): {clean_text[:50]}...")
